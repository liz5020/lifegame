...
## 2026-09-25（佇列批次收尾：全批回歸＋真實模式跨日補點修正）

**〔開發部〕〔測試部〕**
- 全批回歸時發現：真實模式下前端仍會用瀏覽器時間自己補每日池，跟伺服器端(10.3.11)的日期判斷可能不一致。改為：真實模式進入遊戲畫面時若本機台灣日期與上次補點日期不同，向Worker `GET /ap`重新讀餘額(每個日期只問一次)，由伺服器決定補點；mock模式維持本機補點
- 測試：`tests/test-2-action-points.mjs`新增「真實模式跨日：畫面向Worker讀到伺服器補好的5點」，共40項
- 全部測試重跑(`cd tests && node run-all.mjs`，含真實Chromium)：語法檢查通過；批次1 20/20、批次2 40/40、批次3 26/26、批次4 16/16、批次5 10/10、批次6 44/44，全部通過
- `wrangler deploy --dry-run`打包成功(gzip約575KB)；簡轉繁測試全過

---

## 2026-09-25（佇列批次7：公測「已知狀況」公告草稿）

**〔整理〕依使用者佇列批次第7項，只寫文字、不改程式**
- 專案根目錄新增`公告草稿_已知狀況.md`：等待秒數、行動點規則(新人生禮包55點／每把金鑰3次、每天台灣時間00:00補到5點不累加、用完當回合照常寫完)、數值還在調整、復原金鑰遺失找不回來、更新時可能短暫連不上但存檔在伺服器、大改版後舊存檔可能要重開、回報管道(留空)
- 每回合等待秒數目前**沒有真實API實測數字**，先寫「約20～40秒、長的接近1分鐘」並用【】標出，請依實測修改
- 測試：不適用(純文字)

---

## 2026-09-25（佇列批次6：章節成書——人生之書）

**〔開發部〕〔測試部〕依使用者佇列批次第6項實作，對應設計文件十五、`15-人生之書.md`（同日新增，【定案草稿】待確認）**

**index.html**
- `bookAfterTurn()`：每回合成功後把turn_summary與這回合新增的人生大事記(chronicle，含AI回報與程式記錄的大事)放進目前這一章的素材；換階段(高中/大學／技職依`stageMode`，出社會後依`STAGE_LABELS`年齡標籤)、同階段滿5個遊戲年、或人生結束(死亡)時`closeBookDraft()`收成一章
- `processBookQueue()`：背景依序生成(一次一章，不等它、不影響回合)；完成時顯示「📚 新的一章已經寫好」提示；失敗的章節每回合結束自動重試一次、最多3次，之後在人生之書頁面手動「重試」
- 素材`buildChapterMaterial()`：只送摘要、大事、角色名字與關係、上一章章名，最多150則摘要(超過平均抽樣、有大事的回合一定保留)；寫好後刪除素材減少存檔大小
- 人生之書頁面(`openBookPage()`，全螢幕紙本風格)：目錄→點章名閱讀全文；入口在遊玩畫面下方、結局畫面、人生回顧唯讀頁(唯讀不提供重試)
- 「就此闔卷」前等章節寫完(失敗的再試最後一次，最多90秒)再封存；「刪除這段人生」不等
- 反悔：快照包含目前這一章的素材；被反悔掉的那一回合收成的章節撤掉(寫到一半的也撤，寫完後不會跑回書裡)
- mock：`mockGenerateChapter()`依素材拼示意正文；🛠面板新增「模擬章節生成失敗機率」
- 不扣行動點、不動任何遊戲數值

**worker/**
- `prompt.js`：新增`CHAPTER_SYSTEM_PROMPT`(直接切用每回合同一段【寫作規則】`SHARED_WRITING_RULES`，再加章節專用說明)與`CHAPTER_TOOL`(submit_chapter：title/text)
- `worker.js`：`POST /`帶`kind:"chapter"`走`handleChapter()`：驗證素材格式(≤60,000字)、Worker自己組章節prompt與工具、max_tokens 6000；用量記為chapter類別
- `ap.js`：章節額度——每條人生每成功10回合累積1章額度(最多存10章)，新章節用1章額度，同一章重試不再扣但最多5次；不扣行動點

**驗證（USE_MOCK=true＋假上游，未呼叫真實API）**：`tests/test-6-book.mjs` 44項
- 通過：高中→大學／技職換階段收成第一章(從開場回合起)；素材只有摘要與大事、不含完整敘事；大事記含程式記錄的大事
- 通過：背景生成完成(章名＋≥1500字)、不扣行動點、不動遊戲數值、「新的一章已經寫好」提示、入口顯示章數與新章、頁面目錄與全文閱讀、打開後清除新章標記
- 通過：生成失敗遊戲照常、下一回合自動重試、最多3次、頁面顯示重試按鈕、手動重試成功
- 通過：反悔撤掉剛收成的章節且寫完後不會跑回書裡；同階段每5年分章、有「（二）」、章節依序；存檔含章節、讀檔時writing改回待寫
- 通過：死亡時最後一段收成一章、結局畫面入口、闔卷前等寫完＋失敗章節再試、封存到Worker成功、人生回顧唯讀頁看得到書且沒有重試鈕
- 通過：真實路徑(假上游)章節請求用章節prompt/submit_chapter/max_tokens 6000、沿用同一套寫作規則、伺服器端行動點不減、用量記為chapter類別
- 通過：繞過前端沒玩過回合就要求成書→402、玩滿10回合可成書一章、馬上再要新章→402、同一章第6次→429、非遊戲素材→400不呼叫AI
- 通過：另用真實Chromium截圖目視確認目錄/章節頁/提示的版面
- 未測試（需要真實API）：章節文筆品質(是否像小說、show-don't-tell、忠於素材不亂加大事)、字數是否落在1500～2500、實際token花費
- **會讓舊存檔跑不動嗎**：不會。舊存檔從下一回合開始收集素材，改版前玩過的回合不會出現在書裡
- ⚠️prompt與Worker都有改，需要重新部署Worker；index.html要重新上傳Pages

---

## 2026-09-25（佇列批次5：等待畫面加上動態效果）

**〔開發部〕〔測試部〕依使用者佇列批次第5項實作，對應設計文件一、1.1.4（同日新增）**
- **index.html**：`renderTurnLoading()`取代原本靜態的`<div class="loading">`——內嵌SVG一支筆沿著波浪墨線來回書寫(`ink-write`/`pen-move`)，文字後面三個點依序淡入淡出(`dot-fade`，間隔0.2秒)，全部純CSS動畫；顏色沿用既有紙本色票(`--ink`/`--ink-soft`/`--paper-card`)
- `updateLoadingTimer()`：等待開始計時，超過`LOADING_SLOW_NOTICE_MS`(15000)把文字換成「這一頁比較長，旁白還在寫」；回合完成時清掉計時器，下一回合重新計時
- `@media (prefers-reduced-motion: reduce)`：動畫全部關閉，改成靜態(墨線畫滿、筆停在線尾、三個點全亮)
- mock延遲改成變數`MOCK_AI_DELAY_MS`(預設450)，🛠面板新增「模擬AI延遲毫秒」欄位，方便測等待畫面
- 新增`tests/browser-5-loading.cjs`：用真的Chromium(Playwright)驗證——jsdom沒辦法計算CSS動畫與媒體查詢

**驗證（USE_MOCK=true，mock延遲拉到20秒，真實Chromium 141）**：10項
- 通過：等待開始顯示原本文字；筆/墨線/三個點都有動畫且第二個點延遲0.2秒；等待畫面沒有遊戲知識或提示文字；墨線dashoffset隨時間改變(動畫確實在跑)
- 通過：14.5秒仍是原本文字、超過15秒換成安撫文字且動畫持續；20秒回合完成後動畫消失、回合數+1；下一回合重新計時
- 通過：系統設定減少動態時，動畫全部關閉、改為靜態
- 通過：語法檢查；另有截圖目視確認筆尖沿著墨線移動
- 未測試：iOS Safari／Android實機(只測了桌面版Chromium的手機寬度390px)
- **會讓舊存檔跑不動嗎**：不會
